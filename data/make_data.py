"""
make_data.py — builds the dataset for Assignment 1 (Parts B and C).

Do NOT edit this file. Run it once:

    python make_data.py

It writes scores_raw.csv next to itself. The random seed is fixed, so every
student gets exactly the same file, and every answer has exactly one correct
result. Standard library only — no numpy or pandas needed to generate it.
"""
